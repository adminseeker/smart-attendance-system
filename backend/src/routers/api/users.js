...
router.patch('/admin/me', auth, async (req, res) => {
  try {
    ...
    const editUser = await User.findOneAndUpdate({ _id: user.id }, updates, {
      new: true,
    }).select('-password -tokens');
    const editAdmin = await Admin.findOneAndUpdate({ user: user.id }, updates, {
      new: true,
    });
...
router.patch('/update/:id', auth, async (req, res) => {
  try {
    ...
    const editTeacherUser = await Teacher.findOneAndUpdate(
      { user: req.params.id },
      updates,
      { new: true }
    );

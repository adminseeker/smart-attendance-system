...
const addStudents = (id,usn)=>{
    return async (dispatch)=>{
        try {
            ...
            const res = await axios.post("/api/classes/"+id+"/students/usn",body,config);
...
const addTeacher = (id,usn)=>{
    return async (dispatch)=>{
        try {
            ...
            const res = await axios.post("/api/classes/"+id+"/teacher/usn",body,config);
            await dispatch(getClassUsers(id));
            return res.data;
        } catch (error) {
            console.log(error);
...
const getClassUsers = (id)=>{
    return async (dispatch)=>{
        try {
            ...
            let users={ClassStudents:ClassStudents.data,ClassTeacher:ClassTeacher.data}
            dispatch({
                type:"GET_USERS",
                users
            })
...
const removeStudent = (class_id,student_id)=>{
    return async (dispatch)=>{
        try {
            ...
            const res = await axios.delete("/api/classes/"+class_id+"/student/"+student_id,config);
            console.log(res.data);
...
const removeTeacher = (class_id,teacher_id)=>{
    return async (dispatch)=>{
        try {
            ...
            const res = await axios.delete("/api/classes/"+class_id+"/teacher/"+teacher_id,config);
            console.log(res.data);
            await dispatch(getClassUsers(class_id));
            return res.data.msg;
...
export {addStudents,addTeacher, getClasses,editClasses,removeClass,addClass,removeStudent,getClassUsers,removeTeacher};
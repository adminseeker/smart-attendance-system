...
const UsersForm = (props) => {
  ...
  const handleAccessID = async () => {
    ...
    const res = await axios.post(
      '/api/attendance/user/' + props.user.user._id,
      undefined,
      { cancelToken: source.token }
    );
    console.log(res);
    if (res.data.msg) {
      props.dispatch(setAlert(res.data.msg, 'warning'));
  ...
  const handleAccessID2 = async () => {
    ...
    const res = await axios.post(
      '/api/attendance/admin/' + props.user.user._id + '/admin_access_id',
      undefined,
      { cancelToken: source.token }
    );
  ...
  const onSubmit = async (e) => {
    ...
    if (arr === null || phone.length !== 10)
      ...
      if (!props.user) {
        ...
        if (props.user.access_id !== access_id)
          updatedItems.access_id = access_id;
          ...
          <Grid container spacing={2}>
            ...
            {props.user && !onClickedKey && (
              <Grid item xs={1}>
                <IconButton
                  color='secondary'
                  aria-label='Add Access ID'
                  onClick={handleAccessID}
                >
                  <VpnKeyRoundedIcon />
                </IconButton>
              </Grid>
            ...
            {props.user && role == 'admin' && !onClickedKey2 && (
              <Grid item xs={1}>
                <IconButton
                  color='secondary'
                  aria-label='Add Admin Access ID'
                  onClick={handleAccessID2}
                >
                  <VpnKeyRoundedIcon />
                </IconButton>
              </Grid>
